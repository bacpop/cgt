...
fn main() {
    ...
    for i in (0..args.n_samples).progress() {
        obs_sample_core.push(poibin_sample(prior_sample_core[i], &completeness, n_genomes, &mut rng, unif_all));
        obs_sample_notcore.push(poibin_sample(prior_sample_notcore[i], &completeness, n_genomes, &mut rng, unif_all));
        obs_sample_rare.push(poibin_sample(prior_sample_rare[i], &completeness, n_genomes, &mut rng, unif_all));
        obs_sample_notrare.push(poibin_sample(prior_sample_notrare[i], &completeness, n_genomes, &mut rng, unif_all));

    }

    let ns = args.n_samples as f64;

    let mut prob_core_as_notcore: Vec<f64> = Vec::with_capacity(n_genomes);
    let mut prob_notcore_as_core: Vec<f64> = Vec::with_capacity(n_genomes);
    let mut prob_rare_as_notrare: Vec<f64> = Vec::with_capacity(n_genomes);
    let mut prob_notrare_as_rare: Vec<f64> = Vec::with_capacity(n_genomes);
